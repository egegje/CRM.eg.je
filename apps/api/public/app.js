...
const HELP_TEXTS = {
  ...
  "task-settings-sending": {
    title: "✉️ Отправка",
    body: `
      <p><b>Это «рубильники на всю систему»</b> — выключаешь здесь, и фича перестаёт работать у всех сотрудников сразу. Не у тебя одного, а у всех. Персональных тумблеров для этих штук нет специально — чтобы не было каши «у меня выключено, у Ули включено».</p>
      <p>Зачем нужно: если что-то пошло не так (бот разогнался, скрипт начал слать спам, OpenAI стал дорого стоить) — щёлкаешь один флаг, и всё встало. Не надо лезть в каждого юзера и снимать галки руками.</p>
      <h4>Отправка писем</h4>
      <p>Выключено — кнопки «Отправить» в почтовом модуле не работают <b>ни у кого</b>. Удобно перед миграцией или когда подозрение на спам-рассылку из скриптов.</p>
      <h4>Уведомления в Telegram</h4>
      <p>Выключено — бот <code>@task_crm_bot</code> не шлёт <b>никому</b> ничего про задачи (ни назначения, ни review-события, ни утренний дайджест). В журнале доставки такие попытки появятся со статусом «пропущено».</p>
      <h4>AI краткое содержание писем</h4>
      <p>Выключено — в списке писем <b>ни у кого</b> не показывается AI-саммари, экономит токены OpenAI. Существующие саммари остаются в базе.</p>
    `,
  },
  "task-settings-vtb": {
  ...
  return `
    <form style="display:flex;flex-direction:column;gap:0;max-width:640px" onsubmit="saveTaskSettings(event)">
      ...
      <div class="settings-card">
        ...
        <p style="margin:-2px 0 12px;font-size:12px;color:var(--text-muted);line-height:1.4">
          ⚠️ Аварийные рубильники на всю систему. Выключаешь здесь — фича отключается <b>у всех сотрудников сразу</b>, а не только у тебя. Удобно, чтобы быстро остановить что-то одним кликом, не лазая в каждого юзера.
        </p>
        ...
        <p>Галка включена — фича работает у всех. Выключена — на паузе у всех.</p>
      </div>
