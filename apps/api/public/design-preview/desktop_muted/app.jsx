...
function DesktopApp() {
  ...
  const [tweaks, setTweaks] = React.useState(() => {
    try { return { ...TWEAKS, ...JSON.parse(localStorage.getItem('crm-desk-muted-tweaks') || '{}') }; }
    catch { return TWEAKS; }
  });
  ...
  const updateTweaks = (patch) => {
    ...
    setTweaks(next);
    localStorage.setItem('crm-desk-muted-tweaks', JSON.stringify(next));
    window.parent.postMessage({ type: '__edit_mode_set_keys', edits: patch }, '*');
  };

  const [section, setSection] = React.useState(() => localStorage.getItem('crm-desk-muted-section') || 'home');
  ...
  React.useEffect(() => { localStorage.setItem('crm-desk-muted-section', section); }, [section]);
...
function TweaksPanel({ tweaks, update, dark }) {
  ...
  const accentList = [
    ...
    { key: 'steel',  label: 'Сталь',   c: ACCENTS.steel.base  },
    { key: 'sage',   label: 'Шалфей',  c: ACCENTS.sage.base   },
    { key: 'clay',   label: 'Глина',   c: ACCENTS.clay.base   },
  ];
  const densities = [
