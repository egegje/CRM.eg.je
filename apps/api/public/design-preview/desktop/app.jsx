...
function DesktopApp() {
  ...
  const [tweaks, setTweaks] = React.useState(() => {
    try { return { ...TWEAKS, ...JSON.parse(localStorage.getItem('crm-desk-tweaks') || '{}') }; }
    catch { return TWEAKS; }
  });
  ...
  const updateTweaks = (patch) => {
    ...
    setTweaks(next);
    localStorage.setItem('crm-desk-tweaks', JSON.stringify(next));
    window.parent.postMessage({ type: '__edit_mode_set_keys', edits: patch }, '*');
  };

  const [section, setSection] = React.useState(() => localStorage.getItem('crm-desk-section') || 'home');
  const [mailboxFilter, setMailboxFilter] = React.useState(null);
  const [cmdOpen, setCmdOpen] = React.useState(false);

  const dark = tweaks.dark;
  const accentName = ACCENTS[tweaks.accent] ? tweaks.accent : 'indigo';
  ...
  React.useEffect(() => { localStorage.setItem('crm-desk-section', section); }, [section]);
    ...
    <DeskCtx.Provider value={{ accent, dark, density, fontScale, listLayout, aiEnabled, t }}>
      <style>{`
        * { box-sizing: border-box; }
        body { margin: 0; font-family: 'Inter Tight', -apple-system, BlinkMacSystemFont, 'SF Pro', 'Helvetica Neue', sans-serif; font-feature-settings: 'ss01', 'cv11'; -webkit-font-smoothing: antialiased; font-size: ${14 * fontScale}px; }
...
function TweaksPanel({ tweaks, update, dark }) {
  const t = dtheme(dark);
  const accentList = [
    { key: 'indigo',  label: 'Индиго',   c: ACCENTS.indigo.base },
    { key: 'violet',  label: 'Фиолет',   c: ACCENTS.violet.base },
    { key: 'pink',    label: 'Роза',     c: ACCENTS.pink.base },
    { key: 'amber',   label: 'Янтарь',   c: ACCENTS.amber.base },
    { key: 'emerald', label: 'Изумруд',  c: ACCENTS.emerald.base },
    { key: 'sky',     label: 'Небо',     c: ACCENTS.sky.base },
